...
int binary_search_gt_pure(colKey_t *c, int key, int start, int end){
    bool found = false;
    int pos = binary_search_pure(c, key, start, end, &found);
    if(found) {
...
const colKey_t& BPNode::getKey(const int position) {
    return m_keys[position];
}


void BPNode::removeKey(const int position) {
...
BPNode* const BPNode::getPointer(const int position) {
    return m_pointers[position];
}

void BPNode::removePointer(const int position) {
...
rowId_t LeafNode::getRowId(const colKey_t& key) const {
    bool foundKey = false;
    int position = binary_search(m_currentOffset, key, 0, m_currentNumberOfEntries - 1, &foundKey);
    if(foundKey) {
...
rowId_t LeafNode::getGTE(const colKey_t& key) const {
    int position = binary_search_gte(m_currentOffset, key, 0, m_currentNumberOfEntries - 1);
...
BPNode* BPNode::split(BPNode*& root) {
    ...
    int leftSize = BPNode::m_maxNumberOfEntries / 2;
    ...
    while((int) m_currentNumberOfNodes > leftSize + 1) {
...
const LeafNode* BPNode::lookup(const colKey_t& key) {
    if(m_nodeType == LeafNodeType) {
        ...
        int position = binary_search_gt_pure(m_keys, key, 0, m_currentNumberOfEntries - 1);
...
BulkBPTree::BulkBPTree(IndexEntry* data, int size) {
...
int BPNode::m_maxNumberOfEntries = 0;
