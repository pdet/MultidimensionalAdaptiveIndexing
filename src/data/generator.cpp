...
int main(int argc, char* argv[]) {
  ...
  // Generate rq queries
  int rq = 1000;
  std::vector<std::vector<float> > lb_queries(rq, std::vector<float>(dimensions, std::numeric_limits<float>::min()));
  std::vector<std::vector<float> > ub_queries(rq, std::vector<float>(dimensions, std::numeric_limits<float>::max()));
  if (atoi(argv[3]) == 3) {
    size_t i = 0;
