...
void SkewedGenerator::generate(std::string table_path, std::string query_path){
    ...
    for(size_t i = 0; i < n_queries; ++i){
        ...
        for(size_t j = 0; j < n_dimensions; ++j){
            lows.at(j) = distr_query(generator_query) - (per_column_selectivity * n_rows);
            highs.at(j) = distr_query(generator_query) + (per_column_selectivity * n_rows);
            cols.at(j) = j;
        }
        workload->append(
