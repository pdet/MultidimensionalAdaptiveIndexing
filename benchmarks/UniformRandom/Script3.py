...
KDTREE_THRESHOLD = '1'  # Only used for KDTree
QUERY_SELECTIVITY = '0.0000001'
SEED = '1098'
...
# Saving Experiments
if os.path.exists("Results3/") != 1:
    os.system('mkdir Results3')

def getFolderToSaveExperiments():
    global PATH
    experimentsList = os.listdir("Results3/")
    aux = 0
    for experiment in experimentsList:
        if aux < int(experiment):
            aux = int(experiment)
    currentexperiment = aux + 1
    PATH = "Results3/" + str(currentexperiment) + '/'
...
def create_output():
    # Saving Experiments
    header = "algorithm;repetition;query_selectivity;query_number;column_size;column_pattern;number_of_columns;index_creation;index_lookup;scan_time;join_time;projection_time;total_time;n_offsets;offsets_size;n_partitions;n_nodes;index_height;n_leafs;min_partition;max_partition"
