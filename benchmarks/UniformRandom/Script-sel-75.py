...
# Saving Experiments
if os.path.exists("Results-75/") != 1:
    os.system('mkdir Results-75')

def getFolderToSaveExperiments():
    global PATH
    experimentsList = os.listdir("Results-75/")
    aux = 0
    for experiment in experimentsList:
        if aux < int(experiment):
            aux = int(experiment)
    currentexperiment = aux + 1
    PATH = "Results-75/" + str(currentexperiment) + '/'
