...
# Saving Experiments
if os.path.exists("Results-31/") != 1:
    os.system('mkdir Results-31')

def getFolderToSaveExperiments():
    global PATH
    experimentsList = os.listdir("Results-31/")
    aux = 0
    for experiment in experimentsList:
        if aux < int(experiment):
            aux = int(experiment)
    currentexperiment = aux + 1
    PATH = "Results-31/" + str(currentexperiment) + '/'
