...
# Saving Experiments
if os.path.exists("Results-55/") != 1:
    os.system('mkdir Results-55')

def getFolderToSaveExperiments():
    global PATH
    experimentsList = os.listdir("Results-55/")
    aux = 0
    for experiment in experimentsList:
        if aux < int(experiment):
            aux = int(experiment)
    currentexperiment = aux + 1
    PATH = "Results-55/" + str(currentexperiment) + '/'
