...
if benchmark != Benchmarks.GMRQB:
    number_of_attributes = input_or_default(
            """*- How many attributes [default: 5]? """, '5'
    )
...
number_of_tuples = input_or_default(
        """*- How many tuples [default: 1000]? """, '1000'
)
number_of_tuples = ''.join(number_of_tuples.split('.'))
number_of_tuples = int(number_of_tuples)

# Number of queries to be executed
number_of_queries = input_or_default(
        """*- How many queries [default: 1000]? """, '1000'
)
...
# Query selectivity
if benchmark == Benchmarks.GMRQB:
    query_type = input_or_default(
            """*- Query type [default: 7]\
 For a mixed workload choose 8 or more:""",
            '7'
    )
...
algorithms = input_or_default(
    f"""Please specify splitting by spaces [default: 0]: """,
    '0'
).split(' ')
algorithms = list(map(number_to_algorithm, OrderedDict.fromkeys(algorithms)))

repetitions = input_or_default(
        "How many times each algorithm should run [default: 3]?",
        "3"
)
...
# Create config file
with open('./experiments/' + experiment_name + '/config.json', 'w') as file:
...
print(
    ...
    A SQLite file called 'results' will be created,
        with all the measurements stored inside.
